...
> **Note**: サーバー起動後、ブラウザで `http://127.0.0.1:8000/docs` にアクセスすると、Swagger UIによるAPIドキュメントの閲覧・テストが可能です。

### 2. フロントエンド（Macアプリ）の起動

1. Xcodeで `FocusTimer/FocusTimer.xcodeproj` を開きます。
2. ターゲットを `FocusTimer` またはご自身のMacに設定します。
3. `Run` (Cmd + R) を実行してアプリをビルド＆起動します。
